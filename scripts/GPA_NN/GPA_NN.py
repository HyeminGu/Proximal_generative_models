...
yaml_file = "{main_dir}/configs/{dataset}-{generative_model}.yaml".format(main_dir = main_dir, dataset=p.dataset, generative_model=p.generative_model)
...
Q = tf.constant(X_/rescale_factor, dtype=tf.float32) # constant
P = tf.Variable(Y_/rescale_factor, dtype=tf.float32) # variable
if param['unseen'] == True:
    P_unseen = tf.Variable(Y_unseen/rescale_factor, dtype=tf.float32) # variable
...
if not os.path.exists('../../assets/' + param['dataset']):
    os.makedirs('../../assets/' + param['dataset'])
...
for it in range(1, param['epochs']+1): # Loop for updating particles P
    ...
    if param['unseen'] == True:
        unseen_dPs.append( calc_vectorfield(phi, P_unseen, parameters, NN_par, loss_par, data_par))
        P_unseen, unseen_dPs, dP_unseen = solve_ode(P_unseen, lr_P, unseen_dPs, param['ode_solver'], aux_params) # update P_unseen
        if param['mobility'] == 'bounded':
    ...
    if param['epochs']<=100 or it%param['save_iter'] == 0:
        ...
        elif np.prod(param['N_dim']) >= 1000:  # image data
            FIDs.append( calc_fid(pred=P.numpy(), real=Q.numpy()) )
    
    #if it % 10 == 0:
    #    print(it, comp_times[-1], current_loss, KE_P)
    
    if it % (param['epochs']/10) == 0:
        display_msg = 'iter %6d / time %.2f: loss = %.10f, norm of dW = %.2f, kinetic energy of P = %.10f, average learning rate for P = %.6f' % (it, comp_times[-1], current_loss, dW_norm, KE_P, tf.math.reduce_mean(lr_P).numpy())
        if len(FIDs) > 0 :
        ...
        if param['plot_intermediate_result'] == True:
            data = {'trajectories': trajectories, 'divergences': divergences, 'wasserstein1s':wasserstein1s, 'KE_Ps': KE_Ps, 'comp_times': comp_times, 'FIDs':FIDs, 'X_':X_, 'Y_':Y_, 'X_label':X_label, 'Y_label':Y_label, 'dt': lr_Ps, 'dataset': param['dataset'], 'r_param': r_param, 'vectorfields': vectorfields, 'save_iter':param['save_iter']}
            if param['N_dim'] ==2:
...
result = {'trajectories': trajectories, 'vectorfields': vectorfields, 'divergences': divergences, 'KE_Ps': KE_Ps, 'FIDs': FIDs, 'wasserstein1s': wasserstein1s, 'comp_times': comp_times}
if param['unseen'] == True:
