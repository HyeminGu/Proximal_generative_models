...
N_samples=10000 #total number of samples to use
#N_samples=int(sys.argv[1])


epochs = 30000 #number of SGD epochs
...
D_hidden_layers=[128,128, 128] #sizes of hidden layers for the discriminator
G_hidden_layers=[128, 128, 128] #sizes of hidden layers for the generator
...
if di+df==0:
    test_name='../../assets/Learning_student_t'
else:
    test_name='../../assets/Heavytail_submanifold'
...
def sample_Q(m):
    np.random.seed(100)
    X = np.random.standard_cauchy(size=(m, d))
    x = np.abs(X)
    np.random.seed(100)
    y = np.random.uniform(low=0.5, high=2.0, size=(1, d))
    
    return embed_data(np.sign(X)*(x ** y))
...
for it in range(epochs+1):
    if it % SF == 0 or it==100 or it == 500 or (it<=30000 and it % 1000==0):
        ...
        for j in range(N_methods):
            ...
            filename = "%s/%sGAN_%dsamples_0.pickle" % (test_name, method_name, N_samples)
