...
# sample_label: N_samples x num_classes one-hot encoded
    ...
    if 'epochs' not in filename:
        ...
        if epochs == -1:
            ...
            try:
                print("computation time took ", result['comp_times'][epochs-1], "seconds.")
            except:
                print("")
    ...
    for i in range(num_classes):
        if param['N_conditions'] > 1:
            ...
            i_idx = i*print_multiplier + np.arange(print_multiplier)
            #print(i_idx)
    ...
    f = filename.split('.pickle')
    if epochs != -1:
        plt.savefig("%s-%depochs-tiled_image.png"%(f[0], epochs), bbox_inches='tight')
    else:
    ...
    show_or_close_figure(show)
    '''
    #print(len(result['unseen_trajectories']),result['unseen_trajectories'])
    #try:
    if epochs != -1 and param['unseen'] == True:
        samples = result['unseen_trajectories'][int(epochs/save_iter)-1]
        #sample_label = param['Y_unseen_label']
        ...
        for i in range(num_classes):
            if param['N_conditions'] > 1:
                ...
                i_idx = i*num_classes + np.arange(print_multiplier)
        ...
        f = filename.split('.pickle')
        plt.savefig("%s-%depochs-tiled_unseen_image.png" %(f[0], epochs), bbox_inches='tight')
        show_or_close_figure(show)
    #except:
    #    print("")
...
def plot_result(filename, intermediate=False, epochs = 0, iter_nos = None, data = [], show=False):
    ...
    if ('MNIST' in filename  or 'CIFAR10' in filename) and ".pickle" in filename:
        #iter_nos = [0, 1000, 2000, 10000, 1000]
        ...
        if intermediate == False:
            ...
            #plot_tiled_images(print_multiplier=10, samples=None, sample_label=Y_label, epochs = epochs, filename=filename_, show = show)
            #plot_tiled_images(print_multiplier=10, samples=None, sample_label=None, epochs = 1000, filename=filename_, show = show)
            #plot_tiled_images(print_multiplier=10, samples=None, sample_label=None, epochs = 2000, filename=filename_, show = show)
            plot_tiled_images(print_multiplier=10, samples=None, sample_label=None, epochs = 10000, filename=filename_, show = show)
            #plot_tiled_images(print_multiplier=10, samples=None, sample_label=None, epochs = 4000, filename=filename_, show = show)
            #plot_tiled_images(print_multiplier=10, samples=None, sample_label=None, epochs = 5000, filename=filename_, show = show)
            #plot_tiled_images(print_multiplier=10, samples=None, sample_label=None, epochs = -1, filename=filename_, show = show)
